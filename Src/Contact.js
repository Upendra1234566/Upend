...
const Contact = () => {
  ...
  const [contactlist, setCountlist] = useState([]);
  ...
  const ADDCONTACT = async () => {
    //  await AsyncStorage.clear('EMAIL');
    //  await AsyncStorage.clear('PASSWORD');
    navigation.navigate('Login1');
    //console.log(mail + ' ' + Pass);
  };
  const DeleteContact = async index => {
    const temData = contactlist;
    const selectedData = temData.filter((item, ind) => {
      return ind != index;
    });

    setCountlist(selectedData);
    await AsyncStorage.setItem('EMAIL', JSON.stringify(selectedData));

    // Alert.alert('Delete this Data', 'also delete text from device ', [
    //   {
    //     text: 'No',
        ...
        renderItem={({item, index}) => {
          return (
            <View
              ...
              <Text
                style={{marginLeft: 40, color: '#084a01', fontWeight: '700'}}>
